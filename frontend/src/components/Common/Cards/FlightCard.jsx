		...
		<div className='p-5 rounded-xl bg-blue-100 shadow shadow-white flex flex-col gap-5'>
			...
			<div className='flex justify-between items-start'>
				<div>
					...
					<p className='font-serif flex-wrap'>
						{formatWord(segments[0]?.departure.cityName)} ~{' '}
						{formatWord(segments[segments.length - 1]?.arrival.cityName)}
					</p>
			...
			{cancelBooking ? (
				<button
					...
					className='border-2 border-white px-5 py-1 bg-blue-100 w-fit self-center rounded-full font-semibold  text-sm'>
					Cancel
				</button>
			) : (
				<button
