...
const MultiCity = () => {
	...
	const handleOutsideClick = (event) => {
		...
		setFlights((prev) =>
			prev.map((flight) => ({
				...flight,
				isOriginListVisible: originRefs.current[flight.id]?.contains(
					event.target
				)
	...
	const selectDestination = (id, value) => {
		setFlights((prev) =>
			prev.map((flight) =>
				flight.id === id
					? { ...flight, destination: value, isDestinationListVisible: false }
					: flight
			)
		);
	};
	...
	const selectOrigin = (id, value) => {
		setFlights((prev) =>
			prev.map((flight) =>
				flight.id === id
					? { ...flight, origin: value, isOriginListVisible: false }
					: flight
			)
		);
	};
	...
	const handleDestinationChange = (id, value) => {
		setFlights((prev) =>
			prev.map((flight) =>
				flight.id === id
					? { ...flight, destination: value, isDestinationListVisible: true }
					: flight
			)
		);
	};
	...
	const handleOriginChange = (id, value) => {
		setFlights((prev) =>
			prev.map((flight) =>
				flight.id === id
					? { ...flight, origin: value, isOriginListVisible: true }
					: flight
			)
		);
	};
	...
	const handleDateChange = (flightId, selectedDates) => {
		setFlights((prevFlights) =>
			prevFlights.map((flight) =>
				flight.id === flightId
					? { ...flight, departureDate: selectedDates }
					: flight
			)
		);
	};
		...
		<div className='flex flex-col gap-8 w-full'>
			...
			<div
				...
				<div className='w-full h-full relative'>
					<label
						htmlFor='travelers'
						className='absolute left-3 text-sm cursor-text top-[0.1rem] scale-75 -translate-x-2 transform -translate-y-1/2'>
						Travelers
					</label>
					<input
						type='text'
          ...
          `}>
                        ...
                        ${
											...
											onClick={() =>
												handleRoomChange(
													index,
													'adults',
													Math.min(14, rooms[index].adults + 1)
												)
											}>
											+
										</button>
                      ...
                      ${
										...
										onClick={() =>
											handleRoomChange(
												index,
												'children',
												Math.min(6, rooms[index].children + 1)
											)
										}>
										+
									</button>
			...
			{flights.map((flight) => (
									...
									<input
										...
										onFocus={() =>
											setFlights((prev) =>
												prev.map((f) =>
													f.id === flight.id
														? { ...f, isOriginListVisible: true }
														: f
												)
											)
										}
										onChange={(e) =>
											handleOriginChange(flight.id, e.target.value)
										}
										className='px-3 pt-2 w-full'
										autoComplete='off'
									/>
                ...
                ${
								...
								<ul>
									{states.filter((state) =>
										state.toLowerCase().includes(origin.toLowerCase())
									).length ? (
										states
											.filter((state) =>
												state.toLowerCase().includes(origin.toLowerCase())
                      ...
                      ${
									...
									<input
										...
										onChange={(e) =>
											handleDestinationChange(flight.id, e.target.value)
										}
										onFocus={() =>
											setFlights((prev) =>
												prev.map((f) =>
													f.id === flight.id
														? { ...f, isDestinationListVisible: true }
														: f
												)
											)
										}
