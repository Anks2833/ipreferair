...
const DestinationInput = ({ formData, setFormData, locations }) => {
	...
	const toggleDestinationList = () => {
		...
		const sortedLocations = [...locations].sort(
			(a, b) => a.city.localeCompare(b.city) // Sort locations alphabetically by city name
		);
		...
		setFilteredDestinations(
			value.trim() === '' // If input is empty, show all locations
				...
				: sortedLocations.filter(
						(
							location // Filter locations based on input value
						) =>
	...
	return (
		<div
			...
			ref={destinationRef}>
			{' '}
			{/* Wrapper div for the input and dropdown */}
			...
			<div className='border rounded-xl p-3 flex items-center flex-1'>
				...
				<div className='w-full h-full relative'>
					...
					<input
						...
						className='px-3 pt-2 w-full outline-none bg-transparent' // Styling for the input field
