...
const Packages = () => {
		...
		if (selected < 2) {
			return (
				<p className='flex flex-col gap-1 items-center'>
					<Search />
					<p className='font-[500] text-sm'>Please select two or more items</p>
					<p className='text-sm'>
						To start building your trip, select two or more items
					</p>
				</p>
			);
		}
	...
	return (
		<div className='p-4 sm:p-6 md:p-8 flex flex-col gap-8'>
			<div className='flex items-center sm:gap-4 gap-3 text-[0.75rem] text-nowrap flex-wrap w-fit text-[#000000e3] relative  font-semibold'>
				{/* Stay Package Toggle */}
				<p
					onClick={() => setStay(!stay)} // Toggle stay selection on click
					className={
						`border rounded-full px-4 py-2 w-fit  cursor-pointer transition-all duration-300 ease-in-out 
            ...
            ${flight && 'bg-blue-100'}` // Apply background color if flight is selected
				...
				<p
					...
					className={
						`border rounded-full px-4 py-2 w-fit self-baseline max-[400px]:left-0  cursor-pointer transition-all duration-300 ease-in-out
