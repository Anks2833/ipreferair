...
const VerifyEmail = () => {
	...
	const handleVerifyCode = async () => {
		if (otp.length !== 4) {
			// Check if the entered OTP is valid (must be 4 digits)
		...
		try {
			...
			if (res.ok) {
				// If the response is successful, navigate to the password reset page
	...
	useEffect(() => {
		if (timer === 0) {
			// When the timer reaches zero, enable the resend button
			setCanResend(true);
			return;
		}
	...
	useEffect(() => {
		if (modalMessage) {
			// If there's a modal message, set a timeout to clear it
				...
				<div className='pt-16 w-[30rem] max-w-[90%] flex flex-col gap-5'>
					...
					<>
						...
						<p className='text-sm'>
							...
							<b>{currentUser.email}</b>. Check junk mail if it's not in your
							inbox
						</p>
