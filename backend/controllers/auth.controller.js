...
export const signup = async (req, res, next) => {
	...
	try {
		...
		const token = jwt.sign(
			{ id: validUser.email }, // Payload with user ID and admin status
			process.env.JWT_SECRET, // Secret key from environment variables
			{ expiresIn: req.body.keepMeSignedIn ? '7d' : '1d' } // Set token expiration to 7 days if "keepMeSignedIn" is true, else 1 day
		);
		...
		res
			.cookie('access_token', token, {
				...
				maxAge: req.body.keepMeSignedIn
					? 7 * 24 * 60 * 60 * 1000
					: 1 * 24 * 60 * 60 * 1000, // Set expiration of the cookie (7 days or 1 day)
			})
			.status(200);
	} catch (error) {
...
export const resendOTP = async (req, res) => {
	...
	transporter.sendMail(mailOptions, (err, info) => {
		if (err) {
			...
			return res
				.status(500)
				.json({ success: false, message: 'Error resending OTP' }); // Respond with an error if sending fails
		}
...
export const signin = async (req, res, next) => {
	...
	try {
		...
		const token = jwt.sign(
			{ id: validUser.email }, // Payload with user ID and admin status
			'1234', // Secret key from environment variables
		...
		res
			...
			.status(200)
			.json(rest); // Send the response with status 200 and user data (excluding password)
	} catch (error) {
	...
	try {
		...
		if (user.otpExpires > Date.now()) {
			return res.status(400).json({
				success: false,
				message: 'OTP is still valid. Please wait for it to expire.',
			});
		}
		...
		transporter.sendMail(mailOptions, (error, info) => {
			...
			res
				.status(200)
				.json({ success: true, message: 'Code sent to your email.' });
		});
	} catch (error) {
